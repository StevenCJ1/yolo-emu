...
if __name__ == "__main__":
    ...
    # start SimpleTopo
    yoho_topo = SimpleTopo()

    yoho_topo.addController(node_name='c0')
    '''
    yoho_topo.addHostNodes(node_names=['client', 'server']+list(map(lambda x: 'vnf'+str(x), range(n_vnf))),
                        ip_prefix='10.0.0.', ip_suffixes=['12', '15']+list(map(lambda x: str(100+x), range(n_vnf))),
                        dimage='yoho_dev:1', volume=None,
                        docker_args={"cpuset_cpus": '1', 'cpu_quota': 10000})
    '''

    yoho_topo.addHostNodes(node_names=['client', 'server'],
                        ip_prefix='10.0.0.', ip_suffixes=['12', '15'],
                        dimage='yoho_dev:1', volume=None,
                        docker_args={"cpuset_cpus": '0', 'cpu_quota': 30000})
    for i in range(n_vnf):
        yoho_topo.addHostNodes(node_names=['vnf'+ str(i)],
                            ip_prefix='10.0.0.', ip_suffixes=[str(i+1)],
                            dimage='yoho_dev:1', volume=None,
                            docker_args={"cpuset_cpus": str(i//3+1), 'cpu_quota': 30000})


    yoho_topo.addSwitchNodes(node_names=list(
        map(lambda x: 's'+str(x), range(n_vnf))))
    
    # create links, `bw` is bandwith, unit of bandwith is 'Mbit/s'
    #yoho_topo.addLinks(links=['client - '+''.join(list(map(lambda x: 's'+str(x)+'-', range(n_vnf))))+'server'] +
    #                list(map(lambda x: 's'+str(x)+'-'+'vnf'+str(x), range(n_vnf))), bw=1000, delay='10ms', use_htb=True)
    
    yoho_topo.addLinks(links = ['client - s0-s1-s2-server'], bw=1000, delay='10ms', use_htb=True)
    yoho_topo.addLinks(links = ['s0-vnf0'], bw=1000, delay='0ms', use_htb=True)
    yoho_topo.addLinks(links = ['s1-vnf1'],bw=1000, delay='0ms', use_htb=True)
    yoho_topo.addLinks(links = ['s2-vnf2'], bw=1000, delay='0ms', use_htb=True)
    
    yoho_net = yoho_topo.startNetwork()
    
    ## network settings ##
    # delete default flows
    yoho_net.delFlowsOnSwitches(node_names=list(
        map(lambda x: 's'+str(x), range(n_vnf))))
    # add flows
    yoho_net.addFlowsOnSwitch(proto='udp', flows=[
                           'client - ' +
    ...
    yoho_net.disableSwitchCksums(node_nameports=list(
